...
import com.totsp.gwittir.util.impl.WindowContextPersister;
...
/**
 * The WindowContext class is a front end for abstract mvc storage, allowing for the storage of
 * key/value pairs that persist on the mvc between browser sessions.
 * 
 * It is built on top of the other persistence systems in the mvc.util package and fails down (with
...
public class WindowContext {
	...
	private native String instrumentModuleFrame() /*-{
		...
		if($wnd != window){
			window.onunload = function(){
				try{
					//alert("flush");
					instance.@com.totsp.gwittir.mvc.util.WindowContext::flush()();
				} catch(e) {
					alert(e);
				}
			}
	 ...
	 * will happen automatically during onWindowClosing(); doFinal storage may or may not be synchronous
	 * to the call of the flush() method.
	 */
	public void flush() {
		if(flushed != 1){
			persister.storeWindowContextData(data);
			LOG.log(Level.INFO, "Flushed "+data.size()+" items.");
		}
	}
...
//		}
		LOG.log( Level.INFO, "Got persister :"+persister.getClass());
	...
	public void put(String key, String item) {
		...
		LOG.log(Level.FINE, "set ["+key+":"+item+"]");
		data.put(key, item);
	}
