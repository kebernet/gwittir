...
public class BeanMapping {
    static final Class[] BASE_TYPES = {
            java.lang.Integer.class, java.lang.Long.class, java.lang.Byte.class, java.lang.Character.class,
            java.lang.Boolean.class, java.lang.Double.class, java.lang.Float.class, java.lang.String.class,
    ...
    private static void convertCollection(
        IdentityHashMap<Object, Object> instances, Properties mappings, Collection source, Collection destination)
        throws IntrospectionException, ClassNotFoundException, InstantiationException, IllegalAccessException,
            InvocationTargetException, MappingException {
        if (source == null) {
            return;
    ...
    private static Object convertInternal(IdentityHashMap<Object, Object> instances, Properties mappings, Object bean)
        ...
        if (bean.getClass()
                    .isPrimitive() || arrayContains(BASE_TYPES, bean.getClass())) {
            return bean;
        }
        ...
        if (isInterface(Map.class, bean.getClass())) {
            ...
            return list;
        } else if (isInterface(Set.class, bean.getClass())) {
            Set list = (Set) resolveSetType(bean.getClass(), bean.getClass())
                                 .newInstance();
            convertCollection(instances, mappings, (Set) bean, list);

            return list;
        } else if (isInterface(Collection.class, bean.getClass())) {
            Collection collection = (Collection) resolveCollecitonType(bean.getClass(), bean.getClass())
                                                     .newInstance();
        ...
        for (Iterator<String> it = sourceProperties.keySet()
                                                   .iterator(); it.hasNext();) {
            ...
            if (destinationAccessor instanceof Field) {
                ...
                if (isInterface(Map.class, valueClass) && isInterface(Map.class, valueDestinationClass)) {
                    Map map = (Map) resolveMapType(valueClass, valueDestinationClass)
                                        .newInstance();
                    ...
                    List list = (List) resolveListType(valueClass, valueDestinationClass)
                                           .newInstance();
                    convertCollection(instances, mappings, (List) valueObject, list);
                    f.set(dest, list);
                } else if (isInterface(Set.class, valueClass) && isInterface(Set.class, valueDestinationClass)) {
                    Set set = (Set) resolveSetType(valueClass, valueDestinationClass)
                                        .newInstance();
                    convertCollection(instances, mappings, (Set) valueObject, set);
                    f.set(dest, set);
                } else if (
                    isInterface(Collection.class, valueClass) && isInterface(Collection.class, valueDestinationClass)) {
                    ...
                    (valueDestinationClass == resolveClass(mappings, valueClass)) ||
                ...
                if (isInterface(Map.class, valueClass) && isInterface(Map.class, valueDestinationClass)) {
                    ...
                    convertCollection(instances, mappings, (Collection) valueObject, collection);
                    pd.getWriteMethod()
                      .invoke(dest, collection);
                } else if (valueClass == valueDestinationClass) {
                    pd.getWriteMethod()
                      .invoke(dest, valueObject);
                } else if (
                    (valueDestinationClass == resolveClass(mappings, valueClass)) ||
                        (valueDestinationClass.isArray() && valueClass.isArray())) {
                    pd.getWriteMethod()
                      .invoke(dest, convertInternal(instances, mappings, valueObject));
                } else {
                    continue;
                }
            }
    ...
    private static void convertMap(
        IdentityHashMap<Object, Object> instances, Properties mappings, Map source, Map destination)
        throws IntrospectionException, ClassNotFoundException, InstantiationException, IllegalAccessException,
            InvocationTargetException, MappingException {
        if (source == null) {
            return;
        ...
        for (Iterator<Entry<Object, Object>> it = source.entrySet()
                                                        .iterator(); it.hasNext();) {
    ...
    private static HashMap<String, Object> inspectObject(Object o)
        ...
        for (PropertyDescriptor pd : pds) {
            if (
                pd.getName()
                      .equals("class") || (pd.getReadMethod() == null) || (pd.getWriteMethod() == null) ||
                    ((pd.getReadMethod()
        ...
        for (Field field : o.getClass()
                            .getFields()) {
            if (
                ((field.getModifiers() & Modifier.PUBLIC) != 0) && ((field.getModifiers() & Modifier.FINAL) == 0) &&
