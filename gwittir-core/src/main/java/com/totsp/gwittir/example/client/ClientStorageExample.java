...
public class ClientStorageExample extends BoundVerticalPanel<Object> {
    ...
    public ClientStorageExample() {
        ...
        cols[0] = new Field(
                "name", "Name", null, "A name", null,
                new CompositeValidator().add(NotNullValidator.INSTANCE).add(NAME_VALIDATOR),
                new PopupValidationFeedback(PopupValidationFeedback.BOTTOM));
        cols[1] = new Field("value", "Value", null, "The Value of the field");
        table = new BoundTable(
                BoundTable.HEADER_MASK + BoundTable.NO_SELECT_COL_MASK + BoundTable.NO_SELECT_ROW_MASK, factory, cols,
                this.model);
