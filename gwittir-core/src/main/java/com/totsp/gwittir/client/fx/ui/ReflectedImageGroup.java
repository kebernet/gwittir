...
public class ReflectedImageGroup<T> extends AbstractBoundCollectionWidget<T, String> {
    ...
    public ReflectedImageGroup(
        final int baseWidth, final int baseHeight, final double reflectHeight, final double opacity) {
        super();
        ...
        this.grid.getRowFormatter()
                 .setVerticalAlign(0, HasVerticalAlignment.ALIGN_MIDDLE);
        this.imagesPanel.getRowFormatter()
                        .setVerticalAlign(0, HasVerticalAlignment.ALIGN_MIDDLE);
    ...
    public void setValue(Collection<T> value) {
        ...
        this.render((this.value == null) ? new ArrayList()
                                         : this.value);
    }
    ...
    protected void render(Collection value) {
        ...
        for (int i = 0; it.hasNext(); i++) {
            ...
            this.images[i] = new ReflectedImage(
                    this.getRenderer().render(selectObject).toString(), this.baseWidth, this.baseHeight,
                    this.reflectHeight, this.opacity);

            this.imagesPanel.getCellFormatter()
                            .setWidth(0, index, maxSize + "px");
            this.imagesPanel.getCellFormatter()
                            .setHorizontalAlignment(0, index, HasHorizontalAlignment.ALIGN_CENTER);
            this.imagesPanel.setWidget(0, index, this.images[i]);

            this.images[i].addMouseListener(
                new MouseListener() {
