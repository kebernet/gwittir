...
public class WindowContextPersisterHTML5 extends AbstractWindowContextPersister {
    ...
    public void init(final WindowContextCallback listener) {
        db = Databases.openDatabase("gwittir-windowcontext", "1.0", "This gwittir window context database", QUOTA);
        db.run(
            new TransactionTask() {
                public void run(Transaction tx) {
                    tx.execute(
                        "SELECT COUNT(*) FROM windowcontext", null,
                        new ResultsCallback() {
                            public void onSuccess(Transaction tx, ResultSet rs) {
                                tx.execute(
                                    "SELECT key, value FROM windowcontext", null,
                                    new ResultsCallback() {
                                        public void onSuccess(Transaction tx, ResultSet rs) {
                                            for (JavaScriptObjectDecorator d : rs.getRows()) {
                                                loaded.put(d.getStringProperty("key"), d.getStringProperty("value"));
                            ...
                            public void onFailure(Transaction tx, SQLError error) {
                                tx.execute(
                                    "CREATE TABLE IF NOT EXISTS windowcontext (key TEXT, value TEXT)", null,
                                    new ResultsCallback() {
                                        public void onSuccess(Transaction tx, ResultSet rs) {
                                            listener.onInitialized();
                                        }
    ...
    @Override
    public void storeWindowContextData(final Map<String, String> windowContextData) {
        db.run(
            new TransactionTask() {
                public void run(Transaction tx) {
                    for (Entry<String, String> entry : windowContextData.entrySet()) {
                        tx.execute(
                            ...
                            new ResultsCallback() {
                                public void onFailure(Transaction tx, SQLError error) {
                                    Window.alert("Insertion failure " + error.getMessage());
                                }

                                public void onSuccess(Transaction tx, ResultSet rs) {
                                    Logger.getAnonymousLogger()
                                          .log(Level.INFO, "Insertion made.", null);
                                }
                            });
                    }
                }
