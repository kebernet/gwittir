...
public class BoundTable extends AbstractBoundWidget implements HasChunks {
    ...
    private void addRow(Bindable o) {
        ...
        if(((((masks & BoundTable.HEADER_MASK) > 0) && (row >= 3))
                || (row >= 1)) && ((masks & BoundTable.SPACER_ROW_MASK) > 0)) {
    ...
    public Set getSelectedRows() {
        if(this.selectedRowStyles != null) {
            return this.selectedRowStyles.keySet();
        } else {
            HashSet set = new HashSet();
            set.add(new Integer(this.selectedRowLastIndex));
