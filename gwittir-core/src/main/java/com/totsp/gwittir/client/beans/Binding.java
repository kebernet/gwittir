...
public class Binding {
	...
	public Binding(Bindable left, String leftProperty, Validator leftValidator,
			ValidationFeedback leftFeedback, Bindable right,
			String rightProperty, Validator rightValidator,
			ValidationFeedback rightFeedback) {
		this.left = this.createBindingInstance(left, leftProperty);
	...
	private BindingInstance createBindingInstance(Bindable object,
		...
		while (dotIndex != -1) {
			...
			try {
				...
				if (descIndex != -1) {
					descriminator = pname.substring(descIndex + 1, pname
							.indexOf("]", descIndex));
				...
				if (descriminator != null) {
					...
					Object collectionOrArray = INTROSPECTOR.getDescriptor(
							object).getProperty(pname).getAccessorMethod()
							.invoke(object, null);
					object = this.getDiscriminatedObject(collectionOrArray,
							descriminator);
				} else {
					object = (Bindable) INTROSPECTOR.getDescriptor(object)
							.getProperty(pname).getAccessorMethod().invoke(
									object, null);
				}
	...
	private static class DefaultPropertyChangeListener implements
			...
			if (instance.validator != null) {
				try {
					...
					if (instance.feedback != null) {
						...
						instance.feedback.handleException(propertyChangeEvent
								.getSource(), ve);
